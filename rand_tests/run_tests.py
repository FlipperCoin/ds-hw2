...
Your executable should be the compilation result of main2.cpp with your code.
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(BASE_INPUT_DIR):
        print('Running %s...' % filename, flush=True)
        ...
        if RUN_MY:
            ...
            if valgrind_exit_code == 1:
                print('Valgrind errors for %s. Check out valgrind_log.txt' % filename, flush=True)
        if RUN_DIFF:
            ...
            if diff_exit_code != 0:
                print('Difference in outputs for %s. Check out diff_log.txt' % filename, flush=True)
